...
def train(hyp, opt):
    ...
    if opt.enable_modelarts:
        weights = '/cache/checkpoint.ckpt'
        sync_data(opt.ckpt_url, weights)
    ...
    if ema:
        ...
        print("default pretrain weight is currently used for ema_weight.")
    ...
    train_step = create_train_static_shape_fn_gradoperation(model, nc, optimizer, loss_scaler, grad_reducer,
    ...
    for i, data in enumerate(data_loader):
        ...
        print(f"Epoch {epochs}/{cur_epoch}, Step {per_epoch_size}/{cur_step}, size " + str(_p_train_size) + 
                  f"loss: {loss.asnumpy():.4f}, loss_x: {loss_item[0].asnumpy():.4f}, "
                  f"loss_y: {loss_item[1].asnumpy():.4f}, loss_w: {loss_item[2].asnumpy():.4f}, loss_h: {loss_item[3].asnumpy():.4f}, "
                  f"loss_conf: {loss_item[4].asnumpy():.4f}, loss_cls: {loss_item[5].asnumpy():.4f}"
...
def create_train_static_shape_fn_gradoperation(model, nc, optimizer, loss_scaler, grad_reducer=None, rank_size=8,
    ...
    def forward_func(x, label, sizes=None):
        ...
        pred = model(x)
        loss, loss_itmes = compute_loss(pred, label)
        loss *= rank_size
        return loss, ops.stop_gradient(loss_itmes)
...
if __name__ == '__main__':
    opt = get_args_train()
    if opt.enable_modelarts:
        home_path = '/home/work/user-job-dir/V0001'
        opt.data = os.path.join(home_path, opt.data[2:])
        opt.cfg = os.path.join(home_path, opt.cfg[2:])
        opt.hyp = os.path.join(home_path, opt.hyp[2:])
